...
def is_ticket_staff(member: discord.Member) -> bool:
    ...
    return (
        ...
        member.id == cfg.get("index_staff_id")  # optional legacy support
    )
...
async def setuptick(ctx):
    ...
    await ctx.send("**Setup started**\nReply with IDs only. Type `cancel` to stop.")
...
async def index(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            f"• Open this ticket if you would like Indexing service to finish your index and complete your base.\n\n"
            f"• Pay first before starting indexing.\n\n"
            f"**When opening a ticket:**\n"
            f"• Wait for {staff_mention} to answer.\n"
            "• Be nice and patient with staff.\n"
            "• State your Roblox username for the account to index.\n\n"
            "Not following → ticket deleted + 1 hour timeout ♥️"
        ),
        color=discord.Color.blue()
    )
    embed.set_image(url="https://i.ibb.co/JF73d5JF/ezgif-4b693c75629087.gif")
